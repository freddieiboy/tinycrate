...
var ActionBar = React.createClass({
  ...
  footerHeight0: function() {
    return {height: spring(64)}
  },
  footerHeight1: function() {
    return {height: spring(128)}
  },
  loaded: function() {
    const isOpened = !this.state.isOpened;
          ...
          <footer className="homeFooter" style={{backgroundColor: green.lightColor, height: height}}>
            ...
            { this.state.isOpened ? (
              <div className="clearfix" style={{padding: '0 20px 0 20px'}}>
                <input id="message" placeholder='crate message...' className="inputSend" style={styles.inputSend}></input>
              </div>
            ) : (
              null
            )}
